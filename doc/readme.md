...
`注：禁用 ttyd 只影响终端网页的使用，对依赖 ttyd 的插件和 ssh 连接设备操作无任何影响。`
...
1. 安装系统，两种方法：第一种方法连接 ssh，直接输入命令 `openwrt-install-amlogic` 回车，然后按屏幕提示操作。第二种方法登录系统管理页面，在 `系统` - `晶晨宝盒` - `安装 OpenWrt` 页面选择好设备型号，点击 `安装` 按钮，等到页面提示安装完成。

2. 更新系统也有两种方法：第一种方法将升级用的固件文件传输到 N1 的 `/mnt/mmcblk2p4` 目录，接着使用命令 `openwrt-update-amlogic` 按屏幕提示操作。第二种方法登录系统管理页面，在 `系统` - `晶晨宝盒` - `手动上传更新` 页面，点击 `选择文件` - `上传` 按钮将固件上传到设备，上传成功后页面下方会出现 `升级 OpenWrt 固件` 按钮，点击即可。注意两种方法都须先将固件文件解压为 `.img.gz` 格式再上传，否则系统无法识别。
...
1. 安装系统：连接 ssh，输入命令 `install-to-emmc.sh` 回车,然后按屏幕提示操作。

2. 挂载 overlay 分区：iStoreOS 系统可跳过这一步，LEDE 系统进入 `系统` - `挂载点` 页面点击 `生成配置`，下拉到挂载点部分找到对应设备 `/dev/mmcblk1p3` 点击右侧的 `修改` 按钮。在弹出页面中确认启用已勾选，挂载点选择 `作为外部 overlay 使用 （/overlay)` 保存应用。返回到 `挂载点` 页面下拉到底部点击 `保存并应用` 按钮。重启系统生效。

3. 挂载 docker 分区：挂载 docker 分区步骤与 overlay 挂载类似，进入 `系统` - `挂载点` 页面点击 `生成配置`，下拉到挂载点部分找到对应设备 `/dev/mmcblk1p4` 点击右侧的 `修改` 按钮。在弹出窗口中确认启用已勾选，挂载点选择 `作为 docker 空间（/opt)` 保存应用 ( 如果没有该选项就自定义输入 `/opt` )。返回到 `挂载点` 页面下拉到底部点击 `保存并应用` 按钮。重启系统生效。

4. 更新系统：进入 `系统` - `备份与升级` 页面，在 `刷写新的固件` 部分按提示操作即可。注意固件文件须先解压为 `.img.gz` 格式再上传，否则系统无法识别。
