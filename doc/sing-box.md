...
- **2026.04.04** 重新整理 config 配置文件并移除全局屏蔽 QUIC 选项。调整为开启 `advanced.override` 时自动屏蔽 YouTube 的 QUIC 流量。
...
> [!WARNING]
> 1. 安装脚本会**重置** `/etc/sing-box` 目录及 `/etc/config/sing-box` 配置文件。执行前请务必备份个人数据。
...
3. **进阶设置 (basic)** `2025.05.25 更新 整理合并设置条目，调整默认面板`
```config
