...
- 本地配置 0 会按照 `4. 网关相关配置` 设置调整部分配置文件，本地配置 -1 不会对配置文件做任何修改（注意确保 tproxy 监听端口与 `tproxy_port` 设置一致）。
...
- 模板功能会依照模板文件完整替换配置文件 `top-level` 的 objects。且对配置文件的修改拥有最高权限（包括本地配置 -1 也会被修改），可以覆盖服务对配置文件的必要调整，一定要谨慎使用。
