...
export latest_release="$(curl -s https://github.com/openwrt/openwrt/tags | grep -Eo "v[0-9\.]+-*r*c*[0-9]*.tar.gz" | sed -n '/24.10/p' | sed -n 1p | sed 's/.tar.gz//g' | sed 's/v//g')"

# 开始克隆仓库，并行执行
git clone -b v$latest_release --depth 1 $openwrt_repo openwrt &
