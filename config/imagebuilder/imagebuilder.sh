...
custom_packages() {
    ...
    for name in ${packages_list}; do
        ...
        down_url=$(curl -s ${github_api}/${packages_repo}/releases | grep "browser_download_url" | grep -oE "${jq_rule}" | head -n 1)
        ...
        custom_packages_list="${custom_packages_list} ${packages_name}"
    done
